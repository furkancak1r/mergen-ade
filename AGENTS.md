...
- **Shortcuts send a second Enter after a short delay for confirmation.** After sending the command with the first Enter, schedule a second Enter after `TERMINAL_SHORTCUT_SECOND_ENTER_DELAY_MS` (1200ms) via `pending_second_enter`. This ensures AI CLI tools and shells process the command and any confirmation prompts that may appear. Use `process_pending_second_enters()` in the update loop to handle the delayed sends.

...
- **Primary paste shortcut inserts image paths when clipboard contains an image**: When Smart Input draft or edit field is focused, pressing `Ctrl+V` (or `Cmd+V` on macOS) with an image-only clipboard synthesizes a paste event containing the image file path (or saved bitmap path) so egui TextEdit inserts it as text. This intercepts the raw key event before egui and replaces it with `Event::Paste(image_path)`.

...
- **Smart Input focus redirects terminal shortcuts to the draft field.** When the Smart Input draft (or task-edit) field has keyboard focus, a matched terminal command shortcut is written into `terminal.smart_input.draft` (or `edit_draft` when editing a queued task) instead of being sent to the terminal PTY. The redirect targets the terminal that owns the focused Smart Input field, not necessarily the active terminal. If Smart Input is not visible or not focused, shortcuts follow the normal terminal path.

...
- **Directory search result rows must stay stable after first visible results.** Background subtree loading may continue, but newly discovered matches must not be inserted into the visible result list until the user explicitly updates results.
- **Use a search result cache for visible directory search rows.** Cache visible directory/file paths per project and query; reset it when the query changes, the project is removed, or the directory index is force-refreshed.
- **Do not place dynamic "new results" UI above the result tree.** Any update indicator must not shift existing result rows while the user may be clicking them.
- **Search-triggered deferred loading must not mutate visible result ordering directly.** Apply subtree updates to the snapshot, but render search results from the stable cache until the user chooses to refresh the visible set, except for auto-hydrating descendants under already visible directories whose own names match the query.
- **Keep normal lazy expansion independent from search result caching.** Non-search folder expansion must still queue `DirectoryScanMode::LazySubtree` loads immediately.
- **Visible cached search folders must still hydrate.** If a directory is visible in the cached search result set and is marked `DirectoryNode::is_deferred`, rendering it must still queue `DirectoryScanMode::LazySubtree`; only hidden/background matches should stay out of the visible list until explicit update.
- **Directory-name matches may auto-hydrate descendants.** When a cached result directory's own name matches the query, newly loaded descendants under that directory may be merged into the visible cache automatically. Matches discovered outside already visible self-matching directories must remain pending until the user chooses to update results.

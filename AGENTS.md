...
- **Browser panel toolbar must show a compact scope selector when multiple terminal browsers exist.** When a project has terminals with browser tabs, render small inline buttons (e.g., "Project", "T1", "T2") between the tab strip and the URL toolbar so the user can manually switch visible scope without changing the active terminal.

...
- **Launcher dropdown uses fixed width with inner padding.** The Terminal Manager project header's foreground launcher menu (`styled_launcher_menu_button`) must set a fixed width (`FOREGROUND_LAUNCHER_MENU_WIDTH`) via `ui.set_min_width` / `ui.set_max_width` inside the popup. `ui.available_width()` in egui popups is unbounded and can produce absurdly wide menus. Row backgrounds must be explicitly shrunk by `FOREGROUND_LAUNCHER_MENU_PADDING_X` on both sides so they do not touch the popup border. Top and bottom padding (`FOREGROUND_LAUNCHER_MENU_PADDING_Y`) and a small gap between rows (`FOREGROUND_LAUNCHER_ROW_GAP`) keep the content visually separated from the menu chrome. Launcher display names and commands are truncated with `.truncate()`; hover tooltip shows the full text.

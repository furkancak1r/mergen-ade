...
- **Draft text area resize grip resizes the overall footer**: A diagonal grip in the bottom-right corner of the draft row is allocated as a real layout widget (via `ui.allocate_response`) so egui can route drag events to it without being blocked by the overlapping `TextEdit`. Dragging it updates `SmartInputState::user_height` and resets `draft_user_height` to `None`, causing the draft area to auto-fill the expanded footer. The main footer handle also resets `draft_user_height` on drag so the draft area never stays stuck at a small size while the footer grows.
...
- **Popup menus must have an explicit opaque backing.** Do not rely solely on `Frame::menu` fill; paint a full `SURFACE_BG` rect inside the popup closure before content rows. This prevents background elements from bleeding through gaps or margins.

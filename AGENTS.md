...
- **Queued task rows must reserve fixed action button width before flexible spacer**: Do not call `ui.add_space(ui.available_width())` before rendering Send/Edit/Delete buttons. Compute `remaining = (available_width - action_button_width).max(0.0)` so the three action buttons remain visible and clickable at normal finite footer widths. Text and attachments use the leftover space.
